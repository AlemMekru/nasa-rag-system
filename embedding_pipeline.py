...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def extract_mission_from_path(self, file_path: Path) -> str:
        ...
        if 'apollo' in path_str and '11' in path_str:
            return 'apollo_11'
        elif 'apollo' in path_str and '13' in path_str:
            return 'apollo_13'
    ...
    def scan_text_files_only(self, base_path: str) -> List[Path]:
        ...
        logger.info(f"Scanning base path: {base_path.resolve()}")

        if not base_path.exists():
            logger.error(f"Base path does not exist: {base_path.resolve()}")
            return []

        files_to_process = list(base_path.rglob("*.txt"))
        ...
        for file_path in filtered_files[:10]:
            logger.info(f"Sample file: {file_path}")
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        #selected_files = [f for f in files if "challenger" in str(f).lower()]
        ...
        for file_path in selected_files:
            try:
                ...
                result = self.add_documents_to_collection(docs, file_path, update_mode=update_mode)
